...
### 2.5 Check Status Entry

- Verify this PRP has an entry in project `_STATUS.md`
...
### 3.3 Check Status Entry

- Simple PRPs that are "Complete" do NOT need an entry in project _STATUS.md (they're historical)
- Simple PRPs that are "In Progress" or "Not Started" SHOULD have an entry
- Flag active simple PRPs missing from project status
...
## STEP 5: AUDIT PROJECT _STATUS.md
...
- List all section folders: `knowledge-base/concepts/`, `knowledge-base/flows/`, `knowledge-base/implementations/`, `knowledge-base/gotchas/`, `knowledge-base/decisions/`
...
### 5.6.1 Check Project CLAUDE.md Staleness
...
4. Flag if: A completed PRP introduced patterns but no relevant updates appear in CLAUDE.md

### 5.6.2 Check Project PLANNING.md Staleness

For completed PRPs:

1. Read the project's root `PLANNING.md`
...
PROJECT DOCUMENTATION:
  CLAUDE.md: Up to date | Potentially stale
  PLANNING.md: Up to date | Potentially stale

{If issues found:}
PROJ-1: CLAUDE.md may be stale — PRP "{name}" added module but not documented
PROJ-2: PLANNING.md may be stale — PRP "{name}" changed schema but not documented
```
...
- Update project `_STATUS.md` entries (status, phase, counts)
...
4. **Project _STATUS.md last** (after all source data is corrected)
...
- Project _STATUS.md counts are correct
...
5. **ORDER MATTERS** — Fix PRP files first, project _STATUS.md last
