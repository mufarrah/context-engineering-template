...
|----------------|---------|----------|
| New code patterns established | `CLAUDE.md` | "Always use helper_function() for X" |
| File organization changes | `CLAUDE.md` | "Delete modals go in components/modals/" |
| New commands or utilities | `CLAUDE.md` | "New script: npm run sync-types" |
| Anti-patterns discovered | `CLAUDE.md` | "Never call X without Y" |
| Architectural decisions | `PLANNING.md` | "Events now sync via webhook pattern" |
| New data flows | `PLANNING.md` | "Record creation now triggers notification" |
| Integration points added | `PLANNING.md` | "New API endpoint /api/sync/report" |
| Data model changes | `PLANNING.md` | "Added participants table" |

### 5.3 Update CLAUDE.md

Read the root `CLAUDE.md` file, then **APPEND** or **UPDATE** appropriate sections.
...
### 5.4 Update PLANNING.md

Read the root `PLANNING.md` file, then **UPDATE** appropriate sections.
...
PROJECT DOCUMENTATION:
  ...
  - PLANNING.md: {Updated sections or "No changes needed"}
...
- **NEVER** remove existing content from CLAUDE.md or PLANNING.md
- **ALWAYS** regenerate indexes after knowledge base changes
- **ALWAYS** use the correct template for each section type
- **ALWAYS** read CLAUDE.md and PLANNING.md before updating them
