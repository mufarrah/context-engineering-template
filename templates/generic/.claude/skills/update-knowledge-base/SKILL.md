...
| Project-specific patterns (code examples, file org) | `implementations/` | "How to write API routes" |
...
After updating the knowledge base, also update the project's `AGENTS.md`, but ONLY for durable, every-session facts (see gating below).

### 5.1 Identify What Changed

From the PRP or conversation context, determine what areas of the project were modified.

### 5.2 Extract Project Doc Updates

| Knowledge Type | Goes To | Examples |
|----------------|---------|----------|
| New code patterns established | `AGENTS.md` | "Always use helper_function() for X" |
| File organization changes | `AGENTS.md` | "Delete modals go in components/modals/" |
| New commands or utilities | `AGENTS.md` | "New script: npm run sync-types" |
| Anti-patterns discovered | `AGENTS.md` | "Never call X without Y" |
| Architectural decisions | `AGENTS.md` | "Events now sync via webhook pattern" |
| New data flows | `AGENTS.md` | "Record creation now triggers notification" |
| Integration points added | `AGENTS.md` | "New API endpoint /api/sync/report" |
| Data model changes | `AGENTS.md` | "Added participants table" |

**NOTE:** Fix logs, changelogs, per-feature narratives, and transient status must NOT go in `AGENTS.md`. They go to the PRP's own `FIXES.md`/`COMPLETED.md` and the knowledge base (`gotchas/`, `decisions/`, `flows/`, `implementations/`).

### 5.3 Update AGENTS.md (durable facts only)

Read the root `AGENTS.md` file, then **APPEND** or **UPDATE** appropriate sections — but ONLY for durable, every-session facts. Litmus test: "Would a fresh agent need this in EVERY session, regardless of the task?" If no, do not touch AGENTS.md.

**Rules:**
- **APPEND** new items to existing lists/tables
- **UPDATE** existing entries if responsibilities changed
- **UPDATE** diagrams if architecture changed; **ADD** new entries to tables; **APPEND** to lists (don't remove existing items)
- For significant additions, add a comment: `<!-- Added from PRP: {PRP-NAME}, {date} -->`
- Keep formatting consistent with existing content

### 5.4 Skip If No Significant Changes
...
PROJECT DOCUMENTATION:
  - AGENTS.md: {Updated sections (durable facts only) or "No changes needed"}
