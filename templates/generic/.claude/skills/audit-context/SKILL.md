...
### 5.6.1 Check Project AGENTS.md Staleness

For completed PRPs:

1. Read the project's root `AGENTS.md`
2. Search for PRP references (comments like `<!-- Added from PRP: ... -->`)
3. Check if durable, every-session facts from completed PRPs are documented (patterns, conventions, architecture or data-flow changes)
4. Flag if: A completed PRP introduced a durable, every-session fact but no relevant updates appear in AGENTS.md
