...
- [ ] New code patterns from this phase are documented (only if they're durable, every-session conventions)
- [ ] New file organization or module responsibilities are noted (only if structural)
- [ ] New commands or utilities are listed (build/test/lint/dev)
- [ ] Anti-patterns discovered are documented as "always do X / never do Y" rules
- [ ] Architecture or data-flow changes are reflected in the high-level map
- [ ] Fix logs, changelogs, per-feature narratives, and transient status are NOT present (those belong in FIXES.md/COMPLETED.md and the knowledge base)
...
### Project Docs
- [ ] AGENTS.md has durable, every-session facts documented (if any)
