...
Quick reference for all available commands and skills in your Cortex project.
...
| `/populate-knowledge-base` | Full KB discovery and population | Initial setup, after major changes |
| `/rebuild-kb-index` | Regenerate INDEX.md and _SUMMARY.md files | After manual topic edits |
| `/setup-project` | Initial project setup | After copying the Cortex template |
...
- **Just copied the template?** → `/setup-project`
...
4. Designs data model changes, user flows, architecture
...
**Purpose:** Comprehensive project-level health check.

**Detects:** Status mismatches, feature inputs in wrong location, missing KB entries, stale docs, cross-PRP conflicts.
...
**Purpose:** Full discovery and population. Scans the project codebase and all docs.
...
## Project Management Commands (Detailed)
...
**Purpose:** Framework for creating new custom domain skills for your project.
...
| Update KB from PRP | `/update-knowledge-base` | KB + docs | Yes |
| Full KB population | `/populate-knowledge-base` | Entire project | Yes |
| Regenerate indexes | `/rebuild-kb-index` | KB indexes only | Yes |
| Initial setup | `/setup-project` | Entire project | Yes |
...
3. **Follow project safety rules** — Read CLAUDE.md for project-specific safety rules
