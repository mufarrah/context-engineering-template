...
tracking docs + the workspace status, so the next agent picks up exactly where we are — same
...
### 3A.4 — Update the workspace status
...
### 3B.4 — Update the workspace status (if tracked)
