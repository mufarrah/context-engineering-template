...
Check if project-specific `AGENTS.md` files are up to date with completed PRPs.
...
### 5.6.2 Check Project AGENTS.md Staleness

For each project that had completed PRPs:

1. Read the project's `active-projects/{project}/AGENTS.md`
2. Search for PRP references (comments like `<!-- Added from PRP: ... -->`)
3. Check if durable patterns and architecture changes from completed PRPs are documented
4. Flag if: A completed PRP touched this project but no relevant updates appear in AGENTS.md
...
{If issues found:}
PROJ-1: {project}/AGENTS.md may be stale — PRP "{name}" added module but not documented
```
