...
- [ ] New durable, every-session coding conventions or "always/never" rules are documented
...
- [ ] High-level architecture/data-flow changes are reflected

### 5.5.4 If Updates are Missing

Flag as issues:
- "Knowledge base not updated for phase {N}" → run `/update-knowledge-base {PRP path}`
- "Project {name}/AGENTS.md not updated with new durable patterns or architecture changes"
...
### Project Docs
- [ ] Affected project's AGENTS.md has new durable patterns or architecture changes (if any)
